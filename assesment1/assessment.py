...
#initialize enemy stats first and then randomizes for later
e_HP=20
e_SPD=15
e_ATK=10
# randomize here
...
enemy = []  # used as a writable list that gets copies from the constant list ENEMIES
...
def valuecheck(low, hi):  # Start of the function that makes sure user inputs a valid number
    while 1:  # WILL RUN FOREVER UNTIL HAS RETURNED A VALUE
        try:  # for value error detection
            loop = 1
            while loop == 1: # for if input is within given range detection
                choose = int(input(":"))
                if choose < low:
                    print("Try again")
                elif choose > hi:
...
def dead(thing):  # Quick check to make sure whatever thing is dead or not
    if thing <= 0:
        return 1
...
def opening():
    ...
    player = [{"NAME": "", "B_HP": HP, "B_SPD": SPD, "B_ATK": ATK}]  # reinitializes for each reset of the game
    print("\n==============================\nWelcome to New Zealand,\nYour task is to piece together the Treaty of Waitangi\n"
          "that 6 birds have digested as a meal!")
    player[0]["NAME"] = input("\nEnter your name: ")  # Decided not to use any checks whether name is valid or not
...
def fight():
    global enemy
    global treaty_parts  # needs to be global to be able to write to value in order to not be a local variable within the function
    print("==============================\nYOU ARE FIGHTING...")
    ...
    while enemy[0]["HP"] > 0:  # loop for while enemy hp less than 0 but broken= not anymore
        ...
        if choice == 1:  # Attacking selection
            ...
            if enemy[0]["HP"] <= 0:  # Check if killed
                ...
                break  # breaks out to inter so it can detect whether to put player back into battle or end game
            else:
...
def inter():
    global treaty_parts
    while 1:
        if dead(player[0]["B_HP"]) == 1:  # when Player is dead comes to here
